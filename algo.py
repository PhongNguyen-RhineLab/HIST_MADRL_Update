...
# Kiểm tra phiên bản TensorFlow và sử dụng TensorFlow 1.x nếu phiên bản là 2.x
if tf.__version__[0] == '2':
    import tensorflow.compat.v1 as tf

    tf.disable_v2_behavior()

# Đặt seed cho TensorFlow để tái sản xuất kết quả
tf.set_random_seed(1)


# Khởi tạo lớp HIST_Alg
class HIST_Alg(object):
    def __init__(
            self, a_dim, n_actions, s_dim, s_dim_ddpg, s_dim_dqn, a_bound, envSize, model_path_dqn, save_path_ca, mode,
            LR_DQN=0.01,
            LR_A=0.001,  # học số cho actor của ddpg
            LR_C=0.002,  # học số cho critic của ddpg
            GAMMA=1,  # hệ số giảm thưởng
            TAU=0.01,  # thay thế mềm
            BATCH_SIZE=32,
        ...
        self.pointer_ddpg = 0
        self.pointer_dqn = 0
        # Khởi tạo bộ nhớ để lưu trữ các chuyển động trong DDPG
        ...
        # Các tham số placeholder cho DDPG
        ...
        # Khởi tạo session và tải mô hình
        ...
        with g1.as_default():
            ...
            with tf.variable_scope('eval_net'):
                ...
                with tf.variable_scope('l1'):
                    self.w1_eval = tf.get_variable('w1', [self.s_dim_dqn, n_l1], initializer=w_initializer,
                                                   collections=c_names)
            ...
            # Xây dựng hàm mất mát DQN
            with tf.variable_scope('loss'):
                self.loss = tf.reduce_mean(tf.squared_difference(self.q_target, self.q_eval))

            # Khởi tạo toán tử huấn luyện DQN
            ...
            with tf.variable_scope('target_net'):
                ...
                with tf.variable_scope('l1'):
                    self.w1_tar = tf.get_variable('w1', [self.s_dim_dqn, n_l1], initializer=w_initializer,
                                                  collections=c_names)
                ...
                with tf.variable_scope('l3'):
                    self.w3_tar = tf.get_variable('w3', [n_l2, self.n_actions], initializer=w_initializer,
                                                  collections=c_names)
                    self.b3_tar = tf.get_variable('b3', [1, self.n_actions], initializer=b_initializer,
                                                  collections=c_names)
                    self.q_next = tf.matmul(l2, self.w3_tar) + self.b3_tar

            # Khởi tạo session DQN
            self.sess1 = tf.Session(graph=g1)
            saver = tf.train.Saver()
            saver.restore(self.sess1, self.model_path_dqn)

    # Chọn hành động với DQN
    ...
    def learn_ddpg(self):
        ...
        bsDDPG_ = bs_
        bn = bt[:, -1:]  # sử dụng DDPG hay không ở bước tiếp theo
        bd = bt[:, -2:-1]  # đã hoàn thành hay chưa
        ba_ = bt[:, -3:-2]  # lựa chọn mục tiêu ở bước tiếp theo
        q_target = br
